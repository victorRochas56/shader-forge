...
struct VXGISettings {
    ...
    float strength = 1.5f;  // scales the GI term in both modes; 1 = untouched
    ...
    float skyStrength = 1.0f;    // cone-miss sky: direct sky visibility, occlusion-aware
...
struct Mesh {
    ...
    // (Node::lodOverride) deliberately leave it alone, so it stays readable as "what auto wants".
